  ...
  return (
    <>
        ...
        <div className="relative mx-auto w-full max-w-7xl px-5 py-32 sm:px-8">
          <div className="max-w-2xl animate-rise">
            ...
            <p className="mt-6 max-w-xl text-base leading-relaxed text-background/80">
              Premium leather goods cut, stitched and finished by hand at the Government Institute
              for Intellectually Differently Abled. Every purchase funds training, wages and
              independence.
            </p>
        ...
        <dl className="grid gap-8 sm:grid-cols-3 lg:grid-cols-5">
          {impactStats.map((s) => (
            <div key={s.label}>
              ...
              <dd className="mt-2 font-display text-4xl text-foreground">{s.value}</dd>
            </div>
          ))}
        </dl>
      </Section>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((c) => (
            ...
            >
              ...
              <h3 className="mt-2 flex items-center gap-2 font-display text-xl text-foreground">
                {c.name}
                <ArrowRight
        ...
        <div className="grid items-center gap-12 md:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Button asChild variant="quiet" size="pill">
                <Link to="/making-process">See the process</Link>
              </Button>
            </div>
          </div>
        </div>
      ...
      {bestsellers.length > 0 ? (
        <Section className="bg-secondary/40">
          <SectionHeading eyebrow="Bestsellers" title="Most ordered" />
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {bestsellers.map((p) => (
              <ProductCard key={p.id} product={p} />
            ))}
      ...
      <Section className="bg-ink text-background">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="text-3xl leading-tight text-background md:text-5xl">
            Ready to order? A person replies, not a bot.
          </h2>
          <p className="mt-4 text-background/75">
            Tell us the product, quantity and any customisation. We confirm price and timeline on
            WhatsApp.
          </p>
